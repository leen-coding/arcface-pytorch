...
def fit_one_epoch(model_train, model, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val_loss, gen_val_acc, Epoch, cuda, test_loader, lfw_eval_flag, fp16, scaler, save_period, save_dir, local_rank=0):
    total_loss          = 0
    total_accuracy      = 0
...
#计算验证集acc
    ...
    _, _, accuracy_val, _, _, _, _ = evaluate(distances, labels_acc)
    ...
    if local_rank == 0:
        ...
        loss_history.append_loss(epoch, np.mean(accuracy) if lfw_eval_flag else accuracy_val_mean, total_loss / epoch_step, val_total_loss / epoch_step_val)
        print('Total Loss: %.4f' % (total_loss / epoch_step))
        print('Val_acc: %.4f' % accuracy_val_mean)
